...
class ViewController: UIViewController {
    ...
    func translateWord(word: String) {
        if let cache = wordCache[word] {
            print("Translation found in cache:", cache)
            self.myLabel.text = cache
        }
        else {
                ...
                case .success(let jsonData):
                    do {
                        ...
                        if let firstTranslation = translationResponse.def.first?.tr.first?.text {
                            ...
                            self.wordCache[word] = firstTranslation
                        } else {
